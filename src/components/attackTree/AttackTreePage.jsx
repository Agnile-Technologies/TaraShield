...
const AttackTreePage = () => {
  ...
  useEffect(() => {
    if (editId) {
      const treeToEdit = attackTrees.find(tree => tree.id === editId);
      if (treeToEdit) {
        setAttackTree({ name: treeToEdit.name, steps: treeToEdit.steps.join(', '), feasibility: treeToEdit.feasibility });
      }
    }
  }, [editId, attackTrees]);
